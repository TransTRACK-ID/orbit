...
const internalSearchKey = ref(props.searchKey)
...
watch(() => isVisible.value, (isVisible: boolean) => {
  if (props.clearSearchOnToggle) internalSearchKey.value = ''
  isVisible ? (emit('on-show')) : emit('on-hide')
})

const filteredItems = computed(() => {
  return props.items.filter(item => isSameString(item.text, internalSearchKey.value ?? ''))
})
...
function onClickCreate() {
  emit('on-click-add', internalSearchKey.value)
...
function handleMainInput(event: string) {
  ...
  internalSearchKey.value = event
  emit('update:search-key', event)
}

watch(() => props.searchKey, (searchKey) => {
  if (searchKey !== internalSearchKey.value)
    internalSearchKey.value = searchKey
})
...
<template>
  <div class="relative overflow-visible" @focusout="onFocusOut()">
    <div class="relative">
      <TextInput :clearable="props.clearable" :disabled="props.disabled" :required="props.required" :label="props.label"
        :id="props.idTrigger ?? `${props.id}Activator`" :value="isVisible ? internalSearchKey : selectedItem"
        :placeholder="placeholder" @focus.capture="toggleCombobox(true)" :color="color" @on-input="handleMainInput"
        @on-click-clear="onClickClear">
        <template #prefix-above>
          <div v-if="props.isMultiSelect" class="pb-2 gap-2 flex flex-wrap">
            <GeneralChip v-for="value in (props.modelValue as string[])"
              :label="props.items.find(item => item.value === value)?.text" :variant="color">
              <template #suffix>
                <div :class="colorCombobox.hover" class="p-0.5 rounded-full  transition-all cursor-pointer"
        ...
        <template #suffix>
          ...
          <ChevronUp v-if="!$slots.suffix" size="20" class="cursor-pointer transition-transform"
    ...
    <div ref="dropdown" :id="props.idTarget ?? props.id"
      ...
      <div class="bg-white mx-1 rounded-lg border shadow-elevated ">
        <div v-if="!$props.isInlineAdd && !props.plain" class="shadow-sm">
          <button type="button" :class="colorCombobox.text"
            class="flex items-center justify-start w-full py-2.5 px-3.5 text-sm font-[600]  transition hover:bg-gray-100"
        ...
        <div v-if="props.isLoading" class="py-2.5 flex justify-center">
          <svg aria-hidden="true" :class="colorCombobox.loading" class="w-6 h-6 animate-spin  fill-white"
          ...
          <ul v-if="(props.isSearchable ? filteredItems : props.items).length > 0" class="max-h-52 overflow-y-auto">
            <li v-for="(item, i) in (props.isSearchable ? filteredItems : props.items)"
              ...
              <Check
                ...
                size="20" :class="colorCombobox.stroke" />
            </li>
          </ul>

          <div v-else>
            <button v-if="props.isInlineAdd" type="button" class="py-2.5 px-3.5 w-full text-left"
              @click.prevent="onClickCreate">
              <p class="text-sm font-semibold space-x-3">
                <span>Create</span>
                <span class="font-normal py-1 px-3  rounded" :class="[colorCombobox.text, colorCombobox.bg]">{{
                  searchKey
                  }}</span>
              </p>
            </button>

            <p v-else class="py-3.5 text-center cursor-default text-gray-500" @mousedown.prevent="">
              No Items
            </p>
          </div>
        </div>
