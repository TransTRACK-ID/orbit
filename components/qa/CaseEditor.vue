...
<template>
  ...
  <div v-else class="flex flex-col h-full min-h-0 bg-white border border-surface-200 rounded-xl overflow-hidden">
    <div class="px-4 py-3 border-b border-surface-100 flex items-center gap-2">
      ...
      <button
        type="button"
        class="px-3 py-1.5 rounded-lg bg-surface-900 text-white text-xs font-semibold"
        @click="save"
      >
        Save
      </button>
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-3">
      <div>
        ...
        <input v-model="draft.title" type="text" class="w-full text-sm border border-surface-200 rounded-lg px-3 py-2" />
      </div>

      <div class="grid grid-cols-3 gap-2">
        <div>
          <label class="block text-[10px] font-medium text-surface-500 mb-1">Suite</label>
          <select v-model="draft.suiteId" class="w-full text-xs border border-surface-200 rounded-lg px-2 py-2">
        ...
        <div>
          ...
          <select v-model="draft.priority" class="w-full text-xs border border-surface-200 rounded-lg px-2 py-2">
        ...
        <div>
          ...
          <select v-model="draft.status" class="w-full text-xs border border-surface-200 rounded-lg px-2 py-2">
      ...
      <div>
        ...
        <textarea
          ...
          rows="2"
          class="w-full text-xs border border-surface-200 rounded-lg px-3 py-2"
      ...
      <div>
        ...
        <div v-for="(step, idx) in draft.steps" :key="idx" class="border border-surface-200 rounded-lg p-2.5 mb-2 space-y-1.5">
          ...
          <input v-model="step.action" type="text" placeholder="Action" class="w-full text-xs border border-surface-200 rounded px-2 py-1.5" />
          <input v-model="step.expected" type="text" placeholder="Expected result" class="w-full text-xs border border-surface-200 rounded px-2 py-1.5" />
        </div>
      </div>
    </div>
  </div>
