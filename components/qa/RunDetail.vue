...
function startPolling() {
  stopPolling()
  pollTimer = setInterval(() => emit('refresh'), 8000)
}
  ...
  <div v-else class="flex flex-col h-full min-h-0 bg-white border border-surface-200 rounded-xl overflow-hidden">
    <div class="px-4 py-3 border-b border-surface-100">
      <div class="flex items-center gap-2 mb-1">
        <span class="text-xs px-2 py-0.5 rounded-full font-medium" :class="statusClass(run.status)">
          {{ run.status }}
        </span>
      ...
      <div class="text-xs text-surface-500 flex flex-wrap gap-x-2 gap-y-0.5">
        <span>{{ run._passedCount || 0 }}/{{ run._totalCount || 0 }} passed</span>
        <span v-if="run.targetUrl" class="truncate max-w-[min(100%,20rem)]">· {{ run.targetUrl }}</span>
        <span v-if="run.agent">· {{ run.agent.name }}</span>
      </div>
      <p v-if="run.summary && run.status !== 'failed'" class="text-xs text-surface-600 mt-2 max-w-prose">
        {{ run.summary }}
      </p>
    </div>
    ...
    <QaRunAgentExecution
      v-if="workspaceSlug"
      :run="run"
      :agent-execution="run.agentExecution"
      :workspace-slug="workspaceSlug"
    />

    <div class="flex-1 overflow-y-auto p-3 space-y-2">
      <p class="text-xs font-medium text-surface-600 px-1">Test cases</p>
      ...
      >
        <button
          type="button"
          class="w-full flex items-center gap-2 px-3 py-2.5 text-left hover:bg-surface-50"
          @click="expandedId = expandedId === rc.id ? null : rc.id"
        >
          <span class="text-xs px-2 py-0.5 rounded-full font-medium flex-shrink-0" :class="statusClass(rc.status)">
            {{ rc.status }}
          </span>
        ...
        <div v-if="expandedId === rc.id" class="px-3 pb-3 border-t border-surface-100 space-y-3">
          <div class="flex flex-wrap gap-1.5 pt-2">
            <button
              v-for="s in (['passed', 'failed', 'blocked', 'skipped', 'pending'] as QaRunCaseStatus[])"
              :key="s"
              type="button"
              class="text-xs px-2 py-1 rounded-md border disabled:opacity-50"
          ...
          <div v-if="rc.preconditions" class="text-xs text-surface-700 bg-surface-50 rounded-lg p-2.5">
            <span class="font-semibold text-surface-500">Preconditions:</span> {{ rc.preconditions }}
          </div>

          <div v-if="rc.steps?.length" class="space-y-1.5">
            <div v-for="step in rc.steps" :key="step.order" class="text-xs text-surface-600">
              <span class="font-semibold text-surface-700">{{ step.order }}.</span> {{ step.action }}
              <span class="text-surface-400"> → {{ step.expected }}</span>
            </div>
          </div>

          <div v-if="rc.actual" class="text-xs text-surface-700 bg-surface-50 rounded-lg p-2.5 leading-relaxed">
            {{ rc.actual }}
          </div>
          <div v-if="rc.error" class="text-xs text-red-700 bg-red-50 rounded-lg p-2.5 leading-relaxed">
            {{ rc.error }}
          </div>
          ...
          <div v-if="rc.attachments?.length" class="flex flex-wrap gap-2">
            <a
              ...
              target="_blank"
              class="block w-24 rounded-lg overflow-hidden border border-surface-200 bg-surface-50 hover:border-surface-300 transition-colors"
            ...
            >
              <img
                ...
                loading="lazy"
              />
              <span class="block px-1.5 py-1 text-xs text-surface-500 truncate border-t border-surface-100">
                {{ att.originalName }}
              </span>
            </a>
          </div>
