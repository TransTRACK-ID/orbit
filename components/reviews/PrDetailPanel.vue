  ...
  <div class="flex-1 bg-white flex flex-col overflow-hidden min-w-0">
    ...
    <template v-else-if="pullRequest">
      ...
      <div class="px-5 py-4 border-b border-surface-100">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 mb-1">
              ...
              <span
                ...
                class="text-[10px] px-1.5 py-0.5 rounded-full bg-surface-100 text-surface-500 font-semibold"
              >
                Draft
              </span>
              <span
        ...
        <!-- Branch info -->
        <div class="flex items-center gap-2 mt-2 text-[11px] text-surface-500">
          <Icon name="lucide:git-branch" class="w-3.5 h-3.5" />
          <code class="bg-surface-100 px-1.5 py-0.5 rounded text-[10px]">{{ pullRequest.headBranch }}</code>
          <span>→</span>
          <code class="bg-surface-100 px-1.5 py-0.5 rounded text-[10px]">{{ pullRequest.baseBranch }}</code>
        </div>
      </div>
      ...
      <div class="flex-1 overflow-y-auto p-5 space-y-5">
        <!-- Diff -->
        <div>
          <h3 class="text-xs font-semibold text-surface-700 uppercase tracking-wider mb-2">Changes</h3>
          <ReviewsDiffViewer
            :diff="diff"
            :loading="diffLoading"
          />
        ...
        <div v-if="pullRequest.reviewState === 'changes_requested'" class="flex gap-2">
          <button
            class="flex-1 text-xs font-semibold px-3 py-2 rounded-lg bg-primary-600 text-white hover:bg-primary-700 transition-colors flex items-center justify-center gap-1.5"
