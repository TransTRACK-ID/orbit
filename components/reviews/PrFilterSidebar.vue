  ...
  <aside class="w-64 bg-white border-r border-surface-200 flex flex-col overflow-y-auto flex-shrink-0">
    <div class="p-4 border-b border-surface-100">
      <div class="flex items-center justify-between mb-3">
        <h2 class="text-sm font-semibold text-surface-800">Filters</h2>
        <button
      ...
      <!-- Search -->
      <div class="mb-3">
        <label class="block text-[10px] font-semibold text-surface-500 uppercase tracking-wider mb-1">Search</label>
        <div class="relative">
      ...
      <!-- Status -->
      <div class="mb-3">
        <label class="block text-[10px] font-semibold text-surface-500 uppercase tracking-wider mb-1">Status</label>
        <div class="space-y-1">
          <label v-for="s in statusOptions" :key="s.value" class="flex items-center gap-2 cursor-pointer">
            <input
              ...
              type="radio"
              :value="s.value"
              class="w-3 h-3 accent-primary-600"
            />
      ...
      <!-- Review State -->
      <div class="mb-3">
        <label class="block text-[10px] font-semibold text-surface-500 uppercase tracking-wider mb-1">Review State</label>
        <div class="space-y-1">
    ...
    <div class="p-4">
      <button
        class="w-full text-xs font-semibold px-3 py-2 rounded-lg bg-primary-600 text-white hover:bg-primary-700 transition-colors flex items-center justify-center gap-1.5"
        :disabled="loading"
        @click="$emit('refresh')"
      >
