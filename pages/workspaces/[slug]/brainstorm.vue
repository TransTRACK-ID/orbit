          ...
          >
            <div class="flex items-center gap-2 mb-1">
              ...
              <Icon
                ...
                :class="bs.archived ? 'opacity-40' : ''"
              />
              <span class="text-xs font-semibold truncate flex-1" :class="bs.archived ? 'text-surface-400 line-through' : 'text-surface-900'">{{ bs.displayTitle || bs.title }}</span>
      ...
      <div v-if="showCreate" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 backdrop-blur-sm p-4" @click.self="showCreate = false">
        <div class="bg-white rounded-xl border border-surface-200 shadow-lg w-full max-w-md max-h-[85vh] flex flex-col animate-scale-in">
          ...
          <div class="flex-1 overflow-y-auto p-6 pt-4 space-y-4">
            <div>
              <label class="block text-xs font-medium text-surface-600 mb-1.5">Session type</label>
              <div class="flex items-center gap-2">
                <button
                  type="button"
                  class="px-3 py-1.5 text-xs font-medium rounded-lg border transition-colors"
                  :class="sessionMode === 'chat' ? 'bg-surface-900 text-white border-surface-900 dark:bg-black dark:border-black' : 'bg-white text-surface-600 border-surface-200 hover:border-surface-300'"
                  @click="sessionMode = 'chat'; createError = ''"
                >
                  Chat
                </button>
                <button
                  ...
                  :class="sessionMode === 'grill' ? 'bg-amber-500 text-white border-amber-500' : 'bg-white text-surface-600 border-surface-200 hover:border-surface-300'"
                  @click="sessionMode = 'grill'; createError = ''"
                >
              ...
              <p class="text-xs text-surface-400 mt-1.5">
                {{ sessionMode === 'grill'
                  ? 'Structured interview — the agent asks one question at a time with recommended answers'
                  : 'Free-form brainstorm about your codebase' }}
              </p>
            </div>
            ...
            <div v-if="sessionMode === 'grill'">
              <label class="block text-xs font-medium text-surface-600 mb-1.5">Plan or design to grill</label>
              <textarea
                v-model="createInitialPlan"
                rows="4"
                placeholder="Describe your plan, feature idea, or design decision you want stress-tested..."
                class="block w-full text-sm rounded-lg border border-surface-200 bg-white px-3 py-2.5 focus:border-amber-500 focus:ring-1 focus:ring-amber-500 outline-none transition-colors resize-y min-h-[96px]"
              />
              <p class="text-xs text-surface-400 mt-1">The agent will interview you about this plan, one question at a time.</p>
            </div>
          ...
          <div class="flex items-center justify-end gap-2 px-6 py-3">
            ...
            <Button
              ...
              :loading="creating"
              :disabled="!createTitle.trim() || (repoMode === 'new' && !repoFromTemplate) || (sessionMode === 'grill' && !createInitialPlan.trim())"
              @click="handleCreate"
            >
              Create
            </Button>
...
const createInitialPlan = ref('')
const sessionMode = ref<BrainstormMode>('chat')
...
async function handleCreate() {
  ...
  try {
    ...
    if (createdMode === 'grill') {
      chatRunningState.value = true
      await startChat(bs.id)
    }
  } catch (err: any) {
