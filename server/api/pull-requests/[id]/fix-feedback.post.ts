...
export default defineEventHandler(async (event) => {
  ...
  try {
    await db.insert(schema.activityLogs).values({
      ...
      userId: user.id,
      action: 'fix_feedback',
      newValue: { message: `Agent fixing ${comments.length} feedback items from PR review`, pullRequestId: id },
    })
  } catch {}

  return { success: true, taskId, commentCount: comments.length, feedbackLength: feedbackText.length }
})
